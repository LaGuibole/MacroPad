...
// void sendReply(String json)
// {
//     if (pTxChar && pServer->getConnectedCount() > 0) {
//         pTxChar->setValue(json.c_str());
//         pTxChar->notify();
//     }
//     Serial.println("BLE TX: " + json);
...
//     if (cmd == "set") {
//         Serial.println("SET received - profile:" + String(doc["profile"].as<int>()) + " btn:" + String(doc["btn"].as<int>()) + " action:" + doc["action"].as<String>()); // debug
//         int p   = doc.containsKey("profile") ? (doc["profile"].as<int>() - 1) : currentProfile;
//         int btn = doc["btn"].as<int>() - 1;
//         if (p < 0 || p >= NB_PROFILES || btn < 0 || btn >= NB_BUTTONS) {
//             sendReply("{\"ok\":false,\"error\":\"invalid params\"}");
...
//     Serial.println("MacroPad ready — USB HID + BLE UART + OLED");
//     Serial.println("Connections max: " + String(pServer->getConnectedCount()));
//     Serial.println("Advertising: " + String(NimBLEDevice::getAdvertising()->isAdvertising()));
// }
...
//         lastLog = millis();
//         Serial.println("BLE connected: " + String(pServer->getConnectedCount() > 0) + " | Clients: " + String(pServer->getConnectedCount()));
//     }
